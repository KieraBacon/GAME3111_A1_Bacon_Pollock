...
void ShapesApp::BuildShapeGeometry()
{
	...
	submeshData.push_back(SubmeshData{ std::move(box),			"box",			XMFLOAT4(DirectX::Colors::DarkGreen) });
	submeshData.push_back(SubmeshData{ std::move(cylinder),		"cylinder",		XMFLOAT4(DirectX::Colors::SteelBlue) });
	submeshData.push_back(SubmeshData{ std::move(sphere),		"sphere",		XMFLOAT4(DirectX::Colors::Crimson) });
	submeshData.push_back(SubmeshData{ std::move(geosphere),	"geosphere",	XMFLOAT4(DirectX::Colors::DarkRed) });
	...
	for (size_t j = 0; j < submeshData.size(); j++)
	{
		for (size_t i = 0; i < submeshData[j].meshData.Vertices.size(); ++i, ++k)
		{
			...
			vertices[k].Color = submeshData[j].meshColor;
		}
	}
